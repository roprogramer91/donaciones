...
// NOTA: Comentamos la importación de DispositivosConfianza porque ya eliminaste esa lógica.
...
    const existenteDni = await findUserByDni(dni);
    if (existenteDni) {
...
    }
    ...
    // --- 🛑 LÓGICA DE 2FA SUSPENDIDA TEMPORALMENTE ---
    // Si la contraseña es válida, generamos el token final inmediatamente.
    ...
    return res.status(200).json({
      message: "Login exitoso. El 2FA está suspendido temporalmente.",
      token: finalToken, // Se devuelve el token final
      dni: user.dni,
      tipo_usuario: user.tipo_usuario,
    });
    
    /* CÓDIGO ANTERIOR DE 2FA (COMENTADO PARA NO BORRAR LA LÓGICA)
...
    
    // NOTA: Acá se debe agregar la lógica de 'trusted_device_token' cuando se retome.
